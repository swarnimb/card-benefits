...
- [x] [CONSTRAINT-23] reads correctly at 375px inside the centered column — **confirmed live 2026-06-11 in the Task 78 review-gate pass** (roll-up renders cleanly in both collapsed rows and the expanded edit panel at 375px). 1280px not separately pixel-verified, but the app shell is a fixed `max-w-[420px]` centered column (CONSTRAINT-23), so the roll-up renders identically — only the surrounding letterboxing differs.
...
- [x] **VERIFIED live in-app at 375px (2026-06-11):** each corrected benefit's slider max = the per-window value (Resy $100, lululemon $75, Uber Cash $15, Disney-BCP $10, Disney-BCE $7 — confirmed on the Cards usage UI); over-claiming within a single window is impossible (slider hard-capped at per-window max). The Task 76 "→ $Y/yr" roll-up was verified in the review-gate edit row (only non-destructive path to that surface): Resy $100/qtr→$400/yr, lululemon $75/qtr→$300/yr, Uber Cash $15/mo→$180/yr, Hotel $300/6mo→$600/yr; annual benefits correctly show NO roll-up. Verified via a single Platinum refresh → review gate → **Cancel** (never confirmed; `deleteMany` is confirm-only per `confirm/route.ts:150`), so no corrections were touched (post-cancel sliders identical; "last checked 8 days ago" unchanged). Bonus: the fresh Haiku parse produced per-window values directly — end-to-end confirmation that Task 77 parser hardening works in production. (Honors FI-10: exercised the real scrape/edit flow, not static screens.)
