...
## FB19 — Manual Benefit Management

**Date:** 2026-06-12
**Architecture section:** `docs/architecture.md` → Data Model → Manual Benefit Management (Feature 11) and API Routes Summary; `docs/prd.md` Feature 11; `docs/constraints.md` CONSTRAINT-27

**Decided:** You can now add, edit, and delete a benefit by hand — no scrape, no AI, no review-gate confirmation needed (you typed it, so there's nothing to confirm). Every benefit now carries a small "where did this come from?" label, either "scraped" or "manual." The key promise: manual benefits are pinned. A re-scrape that wipes and reloads a card's scraped benefits leaves your hand-entered ones alone. And the moment you hand-edit a scraped benefit, it converts to "manual" — so your edit survives the next re-scrape too, instead of being silently overwritten.

**Means for your product:** When a card has a benefit the scraper misses, or you want to track something the AI didn't pick up, you just add it yourself. Editing a benefit the scraper got slightly wrong now sticks permanently. You're no longer at the mercy of "re-scrape replaces everything" erasing your corrections.

**Why:** Until now the only way a benefit entered the app was through the scrape + AI pipeline, and a re-scrape replaced all of them — so any hand-correction was temporary. Tagging each benefit by source lets the re-scrape target only the scraped ones and leave your manual data untouched. The save logic that writes a benefit was pulled into one shared place so both the scrape-confirm path and the new manual-add path behave identically and can't be tricked into saving fields they shouldn't.

**Check before approving:** This v1 covers usage-tracked benefits only (the spend-against-a-cap kind). Manually adding a "set-and-forget" benefit (the activate-once-and-forget kind) is deliberately out of scope for now — accepted to keep the first version simple.

**What this closes off:** Nothing is removed. Re-scrape still replaces benefits, but now only the scraped ones — the "replace everything, including your manual work" behavior is intentionally gone. Manual set-and-forget benefits stay deferred; adding them later is a straightforward extension of the same source tag.

---

## FB20 — Shareable Demo Mode

**Date:** 2026-06-12
**Architecture section:** `docs/architecture.md` → Infrastructure + Deployment → Demo Mode (Feature 12); `docs/prd.md` Feature 12; `docs/constraints.md` CONSTRAINT-28

**Decided:** There's now a public, read-only, no-login version of the app you can share on a plain URL — no server, no database, no real data behind it. It's a "static export": the app is baked into plain files and published to GitHub Pages, and the pages read pre-baked data snapshots instead of talking to a live backend. The data is entirely fictional and frozen to a fixed date so the demo always looks the same. Anyone can browse all three screens, but every action that would change data is blocked: the harmless interactive bits (like dragging a usage slider) just don't persist, and anything else shows a "Read-only demo" message. A single on/off switch controls all of this — flipped off, the build is identical to the real app.

**Means for your product:** You can send someone a link and they see the full product instantly — no account, no setup, no risk to your real data. It's a portfolio piece and a way to show the product without a live demo environment. Because the demo data is generated by the real engine, what people see is genuinely how the app behaves, not a fake mockup.

**Check before approving:** The demo data is invented and the "current date" is pinned, so anything time-sensitive (what's expiring, what's at risk) reflects that frozen date, not today. That's intentional — it keeps the demo stable — but worth knowing it won't "feel live." The auth screen is skipped entirely in demo mode, by design.

**What this closes off:** Nothing in the real app changes — the demo is fully gated behind one build-time switch, and with it off the build is byte-for-byte the normal app. The static-export approach means the demo can't do anything that needs a server (no real scraping, no saving) — which is the whole point of "read-only," not a limitation to fix later.
